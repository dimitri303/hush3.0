...
// ── GROUNDING + LIGHTING PASS ─────────────────────────
// This pass is intentionally additive and non-invasive.
// It does not move assets or change interactions. It simply helps the existing
// layers feel physically grounded in the same room.
function drawSoftEllipseShadow(x, y, rx, ry, alpha) {
  cx.save();
  cx.globalCompositeOperation = 'multiply';

  const radius = Math.max(rx, ry);
  const g = cx.createRadialGradient(0, 0, 0, 0, 0, radius);
  g.addColorStop(0, `rgba(0,0,0,${alpha})`);
  g.addColorStop(0.45, `rgba(0,0,0,${alpha * 0.45})`);
  g.addColorStop(1, 'rgba(0,0,0,0)');

  cx.translate(x, y);
  cx.scale(1, ry / Math.max(rx, 1));
  cx.fillStyle = g;
  cx.beginPath();
...
function drawTableReflectionPass() {
  if (!layout.table) return;

  const x = layout.table.x;
  const y = layout.table.y;
  const w = layout.table.w;
  const h = layout.table.h;

  cx.save();

  // Clip to the rough tabletop band. The table artwork provides the true edges.
  cx.beginPath();
  cx.rect(x + w * 0.06, y + h * 0.18, w * 0.88, h * 0.36);
  cx.clip();

  cx.globalCompositeOperation = 'screen';

  // City purple/cyan reflected faintly across the glossy surface.
  const cityLine = cx.createLinearGradient(x, y, x + w, y + h * 0.25);
  cityLine.addColorStop(0, 'rgba(140,80,255,0)');
  cityLine.addColorStop(0.42, 'rgba(190,90,255,0.055)');
  cityLine.addColorStop(0.72, 'rgba(90,210,255,0.045)');
  cityLine.addColorStop(1, 'rgba(140,80,255,0)');

  cx.fillStyle = cityLine;
  cx.fillRect(x, y, w, h * 0.45);

  // Small holocube reflection.
  const cubeX = layout.cubeGlow?.x || (layout.cube.x + layout.cube.w / 2);
  const cubeY = layout.cubeGlow?.y || (layout.cube.y + layout.cube.h / 2);

  const cubeReflect = cx.createRadialGradient(
    cubeX,
    cubeY + 34,
    0,
    cubeX,
  ...
  cubeReflect.addColorStop(0, 'rgba(180,120,255,0.13)');
  cubeReflect.addColorStop(0.45, 'rgba(100,220,255,0.055)');
  cubeReflect.addColorStop(1, 'rgba(0,0,0,0)');

  cx.fillStyle = cubeReflect;
  cx.fillRect(x, y, w, h * 0.55);

  // Thin front-edge highlight.
  const edgeY = y + h * 0.33;
  const edge = cx.createLinearGradient(x, edgeY, x + w, edgeY);
  edge.addColorStop(0, 'rgba(255,255,255,0)');
  edge.addColorStop(0.35, 'rgba(255,220,255,0.10)');
  edge.addColorStop(0.60, 'rgba(120,220,255,0.09)');
  edge.addColorStop(1, 'rgba(255,255,255,0)');

  cx.fillStyle = edge;
  cx.fillRect(x + w * 0.08, edgeY, w * 0.84, 2);

  cx.restore();
}

function drawGroundingAndLightingPass() {
  const tp = typeof timeProfile === 'function'
    ? timeProfile()
    : { day: 0, sunset: 0, night: 1 };

  const nightStrength = clamp(tp.night + tp.sunset * 0.55, 0, 1);

  cx.save();

  // 1. Contact shadows. These anchor objects to the rug/floor/table.
  drawSoftEllipseShadow(layout.chair.x + layout.chair.w * 0.48, layout.chair.y + layout.chair.h * 0.88, layout.chair.w * 0.42, 22, 0.45);
  drawSoftEllipseShadow(layout.hifi.x + layout.hifi.w * 0.50, layout.hifi.y + layout.hifi.h * 0.86, layout.hifi.w * 0.42, 18, 0.36);
  drawSoftEllipseShadow(layout.tv.x + layout.tv.w * 0.50, layout.tv.y + layout.tv.h * 0.92, layout.tv.w * 0.42, 16, 0.28);
  drawSoftEllipseShadow(layout.table.x + layout.table.w * 0.50, layout.table.y + layout.table.h * 0.82, layout.table.w * 0.42, 20, 0.22);
  drawSoftEllipseShadow(layout.mug.x + layout.mug.w * 0.50, layout.mug.y + layout.mug.h * 0.92, layout.mug.w * 0.42, 8, 0.18);
  drawSoftEllipseShadow(layout.books.x + layout.books.w * 0.52, layout.books.y + layout.books.h * 0.88, layout.books.w * 0.42, 8, 0.20);
  drawSoftEllipseShadow(layout.remote.x + layout.remote.w * 0.50, layout.remote.y + layout.remote.h * 0.88, layout.remote.w * 0.38, 5, 0.18);
  drawSoftEllipseShadow(layout.cube.x + layout.cube.w * 0.50, layout.cube.y + layout.cube.h * 0.95, layout.cube.w * 0.58, 7, 0.16);

  // 2. Purple city spill from the window onto the floor.
  const citySpill = cx.createRadialGradient(
    layout.win.x + layout.win.w * 0.5,
    layout.win.y + layout.win.h * 1.02,
    20,
    layout.win.x + layout.win.w * 0.5,
    layout.win.y + layout.win.h * 1.02,
    layout.win.w * 0.85
  ...
  citySpill.addColorStop(0, `rgba(190,60,255,${0.14 * nightStrength})`);
  citySpill.addColorStop(0.34, `rgba(110,70,255,${0.08 * nightStrength})`);
  citySpill.addColorStop(0.70, `rgba(80,170,255,${0.035 * nightStrength})`);
  citySpill.addColorStop(1, 'rgba(0,0,0,0)');

  cx.globalCompositeOperation = 'screen';
  cx.fillStyle = citySpill;
  cx.fillRect(0, layout.win.y + layout.win.h * 0.72, RW, RH - layout.win.y);

  // 3. Warm lamp spill over the chair/rug zone.
  if (state.lampOn) {
    const lampX = layout.lamp.x + layout.lamp.w * 0.55;
    const lampY = layout.lamp.y + layout.lamp.h * 0.42;

    const lampGlow = cx.createRadialGradient(
      lampX,
      lampY,
      8,
      lampX + 45,
    ...
    lampGlow.addColorStop(0, 'rgba(255,205,120,0.18)');
    lampGlow.addColorStop(0.32, 'rgba(255,160,80,0.09)');
    lampGlow.addColorStop(0.68, 'rgba(255,120,50,0.035)');
    lampGlow.addColorStop(1, 'rgba(0,0,0,0)');

    cx.fillStyle = lampGlow;
    cx.fillRect(0, layout.lamp.y - 80, RW * 0.55, RH - layout.lamp.y + 100);

    cx.save();
    cx.globalAlpha = 0.18;
    const cone = cx.createLinearGradient(
      layout.lamp.x + layout.lamp.w * 0.55,
      layout.lamp.y + layout.lamp.h * 0.35,
      layout.chair.x + layout.chair.w * 0.70,
      layout.chair.y + layout.chair.h * 0.75
    );

    cone.addColorStop(0, 'rgba(255,220,150,0.45)');
    cone.addColorStop(1, 'rgba(255,180,100,0)');

    cx.fillStyle = cone;
    cx.beginPath();
    cx.moveTo(layout.lamp.x + layout.lamp.w * 0.55, layout.lamp.y + layout.lamp.h * 0.35);
    cx.lineTo(layout.chair.x + layout.chair.w * 0.95, layout.chair.y + layout.chair.h * 0.82);
    cx.lineTo(layout.chair.x + layout.chair.w * 0.30, layout.chair.y + layout.chair.h * 0.92);
    cx.closePath();
    cx.fill();
    cx.restore();
  }

  // 4. TV glow onto the stand and nearby floor.
  if (state.tvOn) {
    const tvCentreX = layout.tv.x + layout.tv.w * 0.45;
    const tvCentreY = layout.tv.y + layout.tv.h * 0.48;

    const tvGlow = cx.createRadialGradient(
      tvCentreX,
      tvCentreY,
      6,
      tvCentreX,
    ...
    tvGlow.addColorStop(0, 'rgba(120,190,255,0.16)');
    tvGlow.addColorStop(0.42, 'rgba(120,90,255,0.07)');
    tvGlow.addColorStop(1, 'rgba(0,0,0,0)');

    cx.fillStyle = tvGlow;
    cx.fillRect(layout.tv.x - 120, layout.tv.y - 40, layout.tv.w + 260, layout.tv.h + 220);

    const tvFloor = cx.createRadialGradient(
      tvCentreX,
      layout.tv.y + layout.tv.h * 1.05,
      8,
      tvCentreX,
      layout.tv.y + layout.tv.h * 1.05,
      160
    ...
    tvFloor.addColorStop(0, 'rgba(110,190,255,0.10)');
    tvFloor.addColorStop(0.55, 'rgba(150,80,255,0.045)');
    tvFloor.addColorStop(1, 'rgba(0,0,0,0)');

    cx.fillStyle = tvFloor;
    cx.fillRect(layout.tv.x - 90, layout.tv.y + layout.tv.h * 0.75, layout.tv.w + 180, 150);
  }

  // 5. Faint table reflections.
  drawTableReflectionPass();

  cx.globalCompositeOperation = 'source-over';

  // 6. Subtle vignette to pull the eye into the centre.
